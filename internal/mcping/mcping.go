// Package mcping implements the Minecraft Java Edition server-list-ping
// protocol (handshake -> status request -> status response -> ping/pong)
// used to measure round-trip latency to a server.
package mcping
...
func StatusRequest(host string, port int, timeout time.Duration, protocolVersion int) Result {
	...
	// Read status response (packet id 0x00)
...
// readPacket reads one length-prefixed packet and splits off its packet id.
